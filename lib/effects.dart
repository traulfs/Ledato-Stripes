...
/// Liefert die Farbe der LED [index] eines Stripes mit [count] LEDs
/// zum Zeitpunkt [t] (Sekunden). [count] ergibt sich aus Länge × Dichte.
///
/// Die Berechnung ist rein deterministisch aus (Strip-Konfiguration, index, t),
/// damit die Simulation ohne Zustand pro LED auskommt.
Color ledColor(LedStrip s, int count, int index, double t) {
  if (!s.enabled || count <= 0) return const Color(0x00000000);
  final n = count;
  final i = s.reversed ? n - 1 - index : index;
  final pos = n == 1 ? 0.0 : i / (n - 1); // 0..1 entlang des Stripes
  // Geschwindigkeit nichtlinear mappen, damit der Regler feinfühlig ist.
  final speed = 0.05 + s.speed * s.speed * 4.0;
  ...
  switch (s.effect) {
    ...
    case EffectType.gradient:
      c = Color.lerp(s.color, s.color2, pos)!;
    ...
    case EffectType.chase:
      ...
      final v = d <= 0 && -d < tail ? math.pow(1 + d / tail, 2).toDouble() : 0.0;
      c = _scale(s.color, 0.03 + 0.97 * v);
    case EffectType.theater:
      final phase = (t * speed * 8).floor() % 3;
      c = (i % 3 == phase) ? s.color : _scale(s.color, 0.04);
    case EffectType.breathe:
      final v = 0.5 - 0.5 * math.cos(t * speed * 2 * math.pi * 0.5);
      c = Color.lerp(_scale(s.color, 0.05), s.color, v)!;
    case EffectType.sparkle:
      final frame = (t * (2 + speed * 20)).floor();
      final h = _hash(i * 7919 + frame * 104729 + s.id.hashCode);
    ...
    case EffectType.confetti:
      ...
      final frac = t * rate - g;
      final h = _hash(i * 6151 + g * 104729 + s.id.hashCode);
      if (h < 0.12) {
    ...
    case EffectType.fire:
      ...
      final f0 = f.floor();
      final n1 = _hash(i * 4813 + f0 * 76801 + s.id.hashCode);
      final n2 = _hash(i * 4813 + (f0 + 1) * 76801 + s.id.hashCode);
      final flicker = n1 + (n2 - n1) * (f - f0);
      final heat =
          ((1.1 - pos * 0.7) * (0.35 + 0.65 * flicker)).clamp(0.0, 1.0);
      c = _heatColor(heat);
  }
  return _scale(c, s.brightness);
}
